...
doc_store = VectorDB(DOCUMENTS)

def retrieve_docs(query: str, threshold: float = 0.4):
    # DocumentStore의 search 메서드 사용
    filtered_results = doc_store.search(query, threshold)

    # 키워드(문자열) 검색: 쿼리 단어가 포함된 문서 추가
    keyword_results = [doc for doc in doc_store.get_documents() if query in doc]

    # 두 결과를 합치고 중복 제거
    results = list(dict.fromkeys(filtered_results + keyword_results))
    print(f"[VectorDB Search Log] 최종 반환 문서: {results}")
    return results
